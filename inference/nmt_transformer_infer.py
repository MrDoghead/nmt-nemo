...
def main():
    ...
    measurements = {}
    count = 0
    with open(args.srctext, 'r') as src_f, MeasureTime(measurements, "total_latency"):
        for line in src_f:
            src_text.append(line.strip())
            if len(src_text) == args.batch_size:
                # print('src:',src_text)
                res = model.translate(text=src_text, source_lang=args.source_lang, target_lang=args.target_lang, measurements=measurements)
