...
def form_attention_mask(input_mask, diagonal=None):
    ...
    attn_mask = input_mask.to(dtype=torch.int32).unsqueeze(1)
    if diagonal is not None:
        ...
        def triu_onnx(x,diagonal):
                    ...
                    if diagonal:
                        arange = arange + diagonal + 1
                    mask = (mask < arange).to(torch.int32)
                    return mask * x
        future_mask = torch.ones(attn_shape, dtype=torch.int32, device=input_mask.device)
        future_mask = triu_onnx(future_mask, diagonal).view(1, input_mask.shape[1], input_mask.shape[1])
